...
const Login = ({ onLoginSuccess, onClose }) => {
  ...
  const handlChange = ({ target: { name, value, type, checked } }) =>
    setFormData((prev) => ({
      ...prev,
  ...
  return (
    <div className="space-y-6 relative">
      <div
        className={`fixed top-4 right-4 z-50 transition-all divide-amber-300 ${showToast ? "translate-y-0 opacity-100" : "-translate-y-20 opacity-0"} `}
      >
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="relative">
          <FaUser className="absolute top-1/2 transform -translate-y-1/2 left-3 text-amber-400" />
          <input
            type="text"
            name="username"
        ...
        <div className="flex items-center">
          <label className="flex items-center">
              <input type="checkbox" name="rememberMe" checked={formData.rememberMe} onChange={handlChange} 
        ...
        <button className="w-full py-3 bg-gradient-to-r from-amber-400 to-amber-600 text-[#2D1B0E] font-bold rounded-lg flex items-center justify-center gap-2 hover:scale-105 transition-transform">
          Sing In <FaArrowRight/>
        </button>
      </form>
      <div className="text-center">
       <Link to='/singup' onClick={onClose} className="inline-flex items-center gap-2 text-amber-400 hover:text-amber-600 transition-colors">
