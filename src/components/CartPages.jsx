...
const CartPages = () => {
  ...
  return (
    <div
      className="min-h-screen overflow-x-hidden py-16 px-4 sm:px-6 lg:px-8 bg-gradient-to-br from-[#1a120b] via-[#2a1e14]
    to-[#3e2b1d] "
    >
      <div className="max-w-7xl mx-auto">
        <h1 className="text-4xl sm:text-5xl md:text-6xl font-bold text-center mb-12 animate-fade-in-down">
          <span className="font-cursive  block text-5xl sm:text-6xl md:text-7xl mb-2 bg-gradient-to-r from-amber-200 to-amber-400 bg-clip-text text-transparent ">
            Your Cart
          </span>
        </h1>
        {cartItems.length === 0 ? (
          <div className="text-center animate-fade-in">
            ...
            <Link
              to="/menu"
              className="transition-all gap-2 hover:gap-3 items-center text-amber-100 inline-flex  duration-300 hover:bg-amber-800/50  bg-amber-900/40 px-6 py-2 rounded-full font-cinzel text-sm uppercase "
            >
              Browser All Items
            </Link>
          </div>
          ...
          <>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
              {cartItems.map((item) => (
                <div
                  key={item.id}
                  className="group bg-amber-900/20
               p-4 rounded-2xl 
               border-4 border-dashed
               transition-all
                border-amber-500 backdrop-blur-sm flex flex-col items-center gap-4 duration-300 hover:border-solid hover:shadow-xl hover:shadow-amber-900/10 transform hover:translate-y-1 animate-fade-in "
                >
                  <div
                  ...
                  <div className="w-full text-center">
                    <h3 className="text-xl font-cursive text-amber-100">
                      {item.name}
                    </h3>
                    <p className="text-amber-100/80 font-cursive  mt-1">
                      ${item.price}
                    </p>
                  </div>
                  ...
                  <div className="flex items-center justify-between w-full">
                   ...
                   <p className="text-sm font-cinzel text-amber-300">
            ...
            <div className="mt-12 pt-8 border-t border-amber-800/30 animate-fade-in-up ">
                 <div className="flex flex-col sm:flex-row justify-between items-center gap-8">
                  ...
                  <div className="flex items-center gap-8">
                     <h2 className="text-3xl font-cursive text-amber-100">
                      Total: ${cartTotal}
                     </h2>
