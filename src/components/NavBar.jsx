...
const renderDesktopAuthButton =() =>{
  return isAuthenticated ? (
      ...
      className="px-3 md:px-3 lg:px-6 py-1.5 md:py-2 lg:py-3 bg-gradient-to-br from-amber-500 to-amber-700 text-[#2D1B0E] rounded-2xl font-bold hover:shadow-lg hover:shadow-amber-600/40 transition-all transform hover:scale-[1.02] border-2 border-amber-600/20 flex items-center gap-2 shadow-md shadow-amber-900/20 text-xs md:text-sm lg:text-sm"
    >
  ...
  const renderMobileAuthBtn = () =>{
    return isAuthenticated ? 
    (
      <button onClick={handleLogout} 
      className="w-full px-4 py-3 bg-gradient-to-bt from-amber-500 to-amber-700 text-[#2D1b0E] rounded-xl font-semibold flex items-center justify-center space-x-2 text-sm">
          <FiLogOut/>
          <span>Logout</span>
      </button>
    ) :(
  ...
  return (
    <nav className="bg-[#2D1B0E] border-b-8 border-amber-900/30 shadow-amber-900/30 sticky top-0 z-50 shadow-[0_25px_50px_-12px] font-vibes group/nav overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 relative">
        <div className="flex justify-between items-center h-16 md:h-20 lg:h-24">
          ...
          <div className="flex-shrink-0 flex items-center space-x-2 group relative md:translate-x-4 lg:translate-x-6 ml-0 md:ml-2">
            <div className="absolute -inset-4 bg-amber-500/10 rounded-full blur-xl opacity-0 group-hover/nav:opacity-100 transition-opacity duration-300"></div>
            ...
            <div className="flex flex-col relative ml-2 max-w-[140px] md:max-w-[160px] lg:max-w-none">
              <NavLink
                to="/"
                className="text-2xl md:text-xl lg:text-4xl bg-gradient-to-r from-amber-400 to-amber-600 bg-clip-text text-transparent  tracking-wider drop-shadow-[0_2px_2px] drop-shadow-black -translate-x-2 truncate md:truncate-none "
              >
                Foodie-Frenzy
              </NavLink>
          ...
          <div className="hidden md:flex items-center space-x-2 md:space-x-1 lg:space-x-4 flex-1 justify-end">
            ...
            <div className="flex items-center space-x-2 md:space-x-3 lg:space-x-4 ml-3 md:ml-3 lg:ml-6 mr-2 md:mr-3 lg:mr-4 ">
              ...
              >
                ...
                {totalItems > 0 && (
                  <span className="absolute -top-2  -right-2 bg-amber-600 items-center justify-center text-amber-100 text-xs w-5 h-5 rounded-full flex">
                    {totalItems}
                  </span>
                )}
              </NavLink>
          ...
          <div className="md:hidden flex items-center mr-2 ">
            <button
              className="text-amber-500 hover:text-amber-300 focus-none transition-all p-2 rounded-xl border-2 border-amber-900/30 hover:border-amber-600/50 relative shadow-md shadow-amber-900/20 hover:shadow-lg hover:shadow-amber-500/30"
            ...
            <div className="pt-4 border-t-2 border-amber-900/30 space-y-2">
                ...
                <FiShoppingCart className="text-lg"/>
                  ...
                  <span className=" top-2  right-2 bg-amber-600
                   items-center justify-center text-amber-100 text-xs w-5 h-5 rounded-full flex">
                    {totalItems}
                  </span>
                )}
                 </NavLink>
  ...
  <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/60 p-4">
    <div className="relative w-full max-w-[480px] rounded-xl border-4 border-amber-700/30 bg-gradient-to-br from-[#2D1B0E] to-[#4a372a] p-6 shadow-[0_0_30px] shadow-amber-500/30">
      
      <button
        onClick={() => navigate("/")}
        className="absolute right-2 top-2 text-2xl text-amber-500 hover:text-amber-300"
      >
        &times;
      </button>

      <h2 className="mb-4 text-center text-2xl font-bold bg-gradient-to-r from-amber-400 to-amber-600 bg-clip-text text-transparent">
        Foodie-Frenzy
      </h2>
