...
export const SingUp = () => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#1a120b] p-4">
      {showToast && (
        <AwesomeToast message="Sing Up Successful" icon={<FaCheckCircle />} />
      )}
      <div className="w-full max-w-md bg-gradient-to-br from-[#2D1B0E] to-[#4a372a] p-8 rounded-xl shadow-lg border-4 border-amber-700/30 transition-all duration-300 hover:shadow-2xl">
        ...
        <form onSubmit={handleSubmite} className="space-y-4">
          ...
          <div className="relative">
            <input
              ...
              required
            />
            <button className="absolute inset-y-0 right-4 flex items-center text-amber-400 hover:text-amber-600 translat-transform hover:scale-125" type="button" onClick={toggleShowPassword}>
        ...
        <div className="mt-6 text-center">
                <Link to='/login' className="group inline-flex items-center text-amber-400 hover:text-amber-600 transition-all duration-300">
                  <FaArrowLeft className="mr-2 -translate-x-2 opacity-0 group-hover:translate-x-0 group-hover:opacity-100 transition-all duration-300"/>
